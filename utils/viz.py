...
def plot_scene_3d(
        ...
        cond_type=None,
        cond=None,
        color_by: str = "none", # "none" | "z" | "height" | "z_top"
        vmin: float = None,
        vmax: float = None,
        lg_type=None,
        dataset='nuplan'
    ):
    """Plots a scene with lanes and agents, colored by height/z for 3D scenes."""
    ...
    for a in range(len(agent_states)):
        ...
        # Draw bounding boxes
        # agent_states format: [x, y, z, vel, cos(heading), sin(heading), length, width, height]
        ...
        if lane_types is None:
            plot_heading_line = True # plot heading line for vehicles, pedestrians, and cyclists
        else:
            plot_heading_line = agent_types[a] in [0, 1]  # Only plot heading line for vehicles and pedestrians, but not static objects
    ...
    if cond_type is not None and cond is not None:
        if cond_type == 'text':
            cond_str = cond
            ax.set_title(cond_str, fontsize=12)
...
def visualize_batch_2d3d(num_samples,
                    agent_samples,
                    lane_samples,
                    agent_types,
                    lane_types,
                    ...
                    seed=0,
                    dataset='nuplan'):
    """ Visualize samples from the batch, using the 3D (height-aware) scene plot when dim='3d'."""
    ...
    f_plot = plot_scene_3d if dim == '3d' else plot_scene
    ...
    for i in range(num_samples):
        ...
        if lane_types is not None:
            ...
            scene_i_lane_types = None
        # extract conditioning info if provided
        if cond_type is not None and cond is not None:
            cond_i = cond[i]
        scene_name = f'epoch_{epoch}_batch_{batch_idx}_sample_{i}_{pred_type}_lg_type_{lg_type[i]}_rank_{rank}_seed_{seed}.png' if raw_file_names is None else raw_file_names[i]
        fig = f_plot(
            scene_i_agents,
            scene_i_lanes,
            scene_i_agent_types,
            ...
            return_fig=save_wandb,
            cond_type=cond_type,
            cond=cond_i if cond_type is not None and cond is not None else None,
        ...
        if cond_type == 'img' and gt and nuplan_data_root is not None and 'cam_infos' in data:
            ...
            if os.path.exists(cond_img_path):
                ...
                save_path = os.path.join(
                    save_dir,
                    f'epoch_{epoch}_batch_{batch_idx}_sample_{i}_{pred_type}_img_rank_{rank}_seed_{seed}.png'
                )
                plt.imsave(save_path, cond_img)
            else:
                print(f"Warning: cond_img_path {cond_img_path} does not exist!")
